// Batman-figur: loader OBJ+MTL (hvis til stede) og eksporterer et visuelt rig
// så game.js kan animere hofte/ben via refs.
...
export function BatmanOBJ({ hipRef, legLRef, legRRef }) {
  ...
  useEffect(() => {
    ...
    mtlLoader.load("assets/Batman/Untitled Model.mtl", (mtls) => {
      ...
      // hvis MTL fejler, lader vi bare fallbacken i game.js tage over (intet at gøre her)
    });
  ...
  return React.createElement('group',{ref:group, position:[0,0,0]},
    // Noder som game.js bruger til at animere
...
// Simpel fallback-figur (vises hvis OBJ ikke er tilgængelig)
